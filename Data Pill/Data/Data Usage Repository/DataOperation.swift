...
extension DataUsageRepository {
    ...
    func getAllData() -> [Data] {
        do {
            ...
            Logger.database.debug("- DATA OPERATION: 📀 Get All Data | ✅ \(result.count) Items")
            return result
            
        } catch let error {
            dataError = DatabaseError.gettingAll(error.localizedDescription)
            Logger.database.error("- DATA OPERATION: 📀 Get All Data | 😭 ERROR: \(error.localizedDescription)")
            return []
        }
    }
    ...
    func getTodaysData() -> Data? {
        do {
            ...
            if dataItems.isEmpty {
                Logger.database.debug("- DATA OPERATION: 📀 Get Today's Data | ✍️ NOT FOUND, Creating...")
                addData(
            ...
            Logger.database.debug("- DATA OPERATION: 📀 Get Today's Data | ✅ CREATED or/and FOUND")
            return todaysData
            
        } catch let error {
            dataError = DatabaseError.gettingTodaysData(error.localizedDescription)
            Logger.database.error("- DATA OPERATION: 📀 Get Today's Data | 😭 ERROR: \(error.localizedDescription)")
            return nil
        }
    }
    ...
    func getDataWithHasTotal() -> Data? {
        do {
            ...
            Logger.database.error("- DATA OPERATION: 📀 Get Data With Has Total | 😭 ERROR: \(error.localizedDescription)")
            return nil
        }
    }
    ...
    func getThisWeeksData(from todaysData: Data?) -> [Data] {
        ...
        do {
            ...
            Logger.database.debug("- DATA OPERATION: 📀 Get This Week's Data | ✅ \(thisWeeksData.count) Items")
            return thisWeeksData
            
        } catch let error {
            dataError = DatabaseError.filteringData(error.localizedDescription)
            Logger.database.error("- DATA OPERATION: 📀 Get This Week's Data | 😭 ERROR: \(error.localizedDescription)")
            return []
        }
    }
    ...
    func getTotalUsedData(from startDate: Date, to endDate: Date) -> Double {
        do {
            ...
            Logger.database.debug("- DATA OPERATION: 📀 Get Total Used Data | ✅ \(totalUsedData) MB of Used Data")
            return totalUsedData
            
        } catch let error {
            dataError = DatabaseError.filteringData(error.localizedDescription)
            Logger.database.error("- DATA OPERATION: 📀 Get Total Used Data | 😭 ERROR: \(error.localizedDescription)")
            return 0
        }
    }
    ...
    func addData(
        ...
        do {
            ...
            Logger.database.debug("- DATA OPERATION: 📀 ADD DATA | ✅ CREATED")
            ...
            Logger.database.error("- DATA OPERATION: 📀 ADD DATA | 😭 ERROR: \(error.localizedDescription)")
        }
    }
        ...
        Future { promise in
            ...
            backgroundContext.performAndWait {
                ...
                do {
                    ...
                    Logger.database.debug("- DATA OPERATION: 📀 ADD BULK DATA | ✅ \(addedIDs.count) Items Created Successfully")
                    ...
                    Logger.database.error("- DATA OPERATION: 📀 ADD BULK DATA | 😭 ERROR: \(error.localizedDescription)")
    ...
    func updateTodaysData(
        ...
        do {
            ...
            /// 4. Update Store
            if isUpdated {
                Logger.database.debug("- DATA OPERATION: 📀 Update Today's Data | ✅ UPDATED")
                updateToLatestData()
            }
            
        } catch let error {
            dataError = DatabaseError.updatingData(error.localizedDescription)
            Logger.database.error("- DATA OPERATION: 📀 Update Today's Data | 😭 ERROR: \(error.localizedDescription)")
        }
    }
        ...
        Future { promise in
            ...
            backgroundContext.performAndWait {
                ...
                do {
                    ...
                    Logger.database.debug("- DATA OPERATION: 📀 UPDATE BULK DATA | ✅ \(updatedIDs.count) Items Updated Successfully")
                    promise(.success(true))
                    
                } catch let error {
                    Logger.database.error("- DATA OPERATION: 📀 UPDATE BULK DATA | 😭 ERROR: \(error.localizedDescription)")
                    promise(.success(false))
                }
            }
        }
        ...
        Future { promise in
            ...
            backgroundContext.performAndWait {
                do {
                    ...
                    Logger.database.debug("- DATA OPERATION: 📀 DELETE ALL DATA | ✅ DELETED")
                    promise(.success(true))
                    
                } catch let error {
                    Logger.database.error("- DATA OPERATION: 📀 DELETE ALL DATA | 😭 ERROR: \(error.localizedDescription)")
                    promise(.success(false))
                }
            }
        }
